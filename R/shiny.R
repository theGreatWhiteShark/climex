...
color.table <- function( x.html.table, css.colours, style = "table-condensed table-bordered" ){
    x.html.table.split <- str_split( x.html.table, "\n" )[[ 1 ]]
  ...
  return( list( tags$script( sprintf( '$( "table" ).addClass( "table %s" );', style ) ),
               HTML( x.html.table ) ) )
}

##' @title likelihood.gui
##' @description Displays a contour plot of the likelihood function of a time series and the optimization routes for a bunch of provided initial points. This debugging tool is supposed to shed some light on failing optimization runs.
##'
...
##' @title climex
##' @description Shiny app combining most of the tools used in extreme value analysis using GEV fitting via maximum likelihood 
##'
...
climex <- function( x.input = NULL ){
    app.dir <- system.file( "climex_app", package = "climex" )
    if ( app.dir == "" )
        stop( "Could not find the directory of the climex app." )
