...
x.thresh.fit <- climex::fit.gpd( x.thresh, total.length = length( temp.potsdam )  )

test_that( "fit.gev has the correct output format", {
          expect_equal( class( x.block.fit ), c( "list", "climex.fit.gev" ) )
...
test_that( "fit.gev's rerun works", {
    expect_equal( as.numeric( climex::fit.gev( x.block, rerun = FALSE )$par ),
...
test_that( "fit.gev's actual fitting works for all methods", {
    expect_equal( as.numeric( climex::fit.gev( x.block,
                 ...
                 c( 11.7269647, 1.4291553, -0.2607239 ) )
    expect_equal( as.numeric( climex::fit.gev( x.block, method = "BFGS" )$par ),
                 c( 11.7269972, 1.4292086, -0.2607638 ) )
    expect_equal( as.numeric( climex::fit.gev( x.block, method = "CG" )$par ),
                 c( 11.7269971, 1.4292080, -0.2607637 ) )
    expect_equal( as.numeric( climex::fit.gev( x.block, method = "SANN" )$par ),
                 c( 11.7269970, 1.4292080, -0.2607637 ), tolerance = 1E-5 )
    expect_equal( as.numeric( climex::fit.gev( x.block, method = "nmk" )$par ),
                 c( 11.7270472, 1.4292892, -0.2607769 ) )
})
test_that( "the handing over of initial parameters works", {
    expect_equal( as.numeric( climex::fit.gev(
        x.block, initial = climex::likelihood.initials( x.block ) )$par ),
        c( 11.7269647, 1.4291553, -0.2607239 ) )
    expect_equal( as.numeric( climex::fit.gev( x.block,
                                              initial = c( 12, 1.8, -.2 ) )$par ),
                 c( 11.7270960, 1.4293212, -0.2608456 ) )
})
test_that( "fit.gev's error estimation works", {
    expect_equal( names( climex::fit.gev( x.block, error.estimation = "MLE" )$se ),
                 c( "location", "scale", "shape", "100.rlevel" ) )
    expect_equal( as.numeric( climex::fit.gev( x.block, error.estimation = "MLE" )$se ),
                 c( 0.1389792, 0.09613785, 0.04381064, 0.06443386 ),
                 tolerance = 1E-6 )
    expect_equal( as.numeric( climex::fit.gev( x.block, error.estimation = "MC",
...
test_that( "fit.gpd's actual fitting works for all methods", {
    expect_equal( as.numeric( climex::fit.gpd( x.thresh,
                                              method = "Nelder-Mead",
                                              total.length = length( temp.potsdam )  )$par ),
                 c( 2.7248551, -0.2493577 ), tolerance = 1E-6 )
    expect_equal( as.numeric( climex::fit.gpd( x.thresh, method = "BFGS",
                                              total.length = length( temp.potsdam )  )$par ),
                 c( 2.7247329, -0.2494032 ), tolerance = 1E-6 )
    expect_equal( as.numeric( climex::fit.gpd( x.thresh, method = "CG",
                                              total.length = length( temp.potsdam )  )$par ),
                 c( 2.724670, -0.249398 ), tolerance = 1E-6 )
    expect_equal( as.numeric( climex::fit.gpd( x.thresh, method = "SANN",
                                              total.length = length( temp.potsdam )  )$par ),
                 c( 2.7248020, -0.2494208 ), tolerance = 1E-5 )
    ## does not work yet
    ## expect_equal( as.numeric( climex::fit.gpd( x.thresh, method = "nmk" )$par ),
    ##              c( 11.7270472, 1.4292892, -0.2607769 ) )
})
test_that( "the handing over of initial parameters works", {
    expect_equal( as.numeric( climex::fit.gpd(
        x.thresh, initial = climex::likelihood.initials( x.thresh, model = "gpd" ),
        total.length = length( temp.potsdam )  )$par ),
        c( 2.7248551, -0.2493577 ) )
    expect_equal( as.numeric( climex::fit.gpd( x.thresh, initial = c( 3, -.3 ),
                                              total.length = length( temp.potsdam )  )$par ),
                 c( 2.7, -1.110223e-17 ) )
})
test_that( "fit.gpd's error estimation works", {
    expect_equal( names( climex::fit.gpd( x.thresh, error.estimation = "MLE",
                                         total.length = length( temp.potsdam ) )$se ),
                 c( "scale", "shape", "100.rlevel" ) )
    expect_equal( as.numeric( climex::fit.gpd( x.thresh, error.estimation = "MLE",
                                              total.length = length( temp.potsdam )  )$se ),
                 c( 0.08358535, 0.01737440, 0.01509256 ),
                 tolerance = 1E-6 )
    expect_equal( as.numeric( climex::fit.gpd( x.thresh, error.estimation = "MC",
...
test_that( "fit.gpd's threshold argument affect the result the way it is supposed to", {
    expect_equal( as.numeric( climex::fit.gpd( x.thresh, threshold = 29,
                                              total.length = length( temp.potsdam )
                                              )$return.level ), 38.05577 )
})
