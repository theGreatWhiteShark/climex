...
data.complete <- parallel::mclapply( data.list, remove.incomplete.years,
                                    mc.cores = parallel::detectCores() )

## Remove all series containing less than 30 years
data.cleaned <- data.complete[ Reduce( c, parallel::mclapply( data.complete, function( x )
    length( unique( year( x ) ) ), mc.cores = parallel::detectCores() ) ) >= 30 ]

## calculate the anomalies of the data
data.anomalies <- parallel::mclapply( data.cleaned, anomalies )
```
...
data.blocked <- parallel::mclapply( data.anomalies, block, mc.cores = parallel::detectCores() )

## fitting of the GEV function using maximum likelihood
data.fitted <- parallel::mclapply( data.blocked, gev.fit, mc.cores = parallel::detectCores() )
