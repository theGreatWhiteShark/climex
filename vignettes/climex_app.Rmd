...
series of
class [xts](https://cran.r-project.org/web/packages/xts/index.html)
and a data.frame with the columns *longitude*, 
*latitude*, *altitude*(not used at this current stage) and *name* of
the individual stations their positions will be marked in the
**Map**-tab.

Alternatively one can also choose to work with artificial data by
setting the *Data base* selector in the sidebar accordingly or to provide a single
...
variety of tools to analyze an individual time series.
...
ones) you can toggle all the points by clicking on them or brush them
...
This radio button controls whether to fit the generalized extreme value
(GEV) distribution (*Blocks*) or the generalized Pareto (GP) one.
...
the choice at *Blocks vs. Threshold*) which becomes very handy
...
*Nelder-Mead* algorithm to be the best method for fit the GEV
parameters.
...
of the fitting procedure and a high quality of the time series. This
...
local minimum during optimization and because of this leading to
...
visually verify if the optimization got stucked at a different location
compared to the other routes or if all of them lead to the same
result.

### Staring points of the optimization routine

As the name suggests this table holds the initial positions the
...
In the table on top of the **Options** box you can see the initial
...
to initialize the optimization. The later ones you can modify too. By
doing so you will also affect all other components depending on the fit
...
weather service downloaded in the [other](./data_dwd_and_usage.Rmd) vignette of this package you
