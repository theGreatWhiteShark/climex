...
The **climex** package is intended to be used for the analysis of
various station data. In order to ease the handling and management of
different data sources, it has an auxiliary function to import R data
files found in the folder stored in the `climex.path` global option.
By default this path points to the folder *~/R/climex/* in your home
directory. If you, instead, have your data stored somewhere else,
you can change the default setting by adding the following lines to
your R configuration file *~/.Rprofile* in your home.
...
The data files, stored in the *.RData* file format, do not have to
reside at the top level of the folder. It will be search recursively.
...
source.data()
```
This function will list all *.RData* files found in directory and loads
the selected one into the environment the function was called
in. Using the *download.path* argument, you can alter the path
searched recursively by the function. This will override the default
setting stored in the *climex.path* option.

# Preprocessing and fitting

Now that we have enough data, let's do some extreme value analysis. 

The climex package at its current stage is intended to provide tools
...
**temp.potsdam**. This way you can follow this tutorial regardless of
your choice whether or not to download the DWD data collection.
...
Just a remark: If we would deal with all the different station data of
the DWD
